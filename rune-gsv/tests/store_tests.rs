...
fn test_default_path_exists_and_store_load_cycle() {
    ...
    let slot = SGLRuneSlot {
        ...
        ranking: rune_gsv::slot::RankingData { query_id: "q1".to_string(), text: "q".to_string(), candidate_ids: vec![], elo_scores: std::collections::HashMap::new() },
    };
    let key = store.store("test intent", slot).unwrap();
    assert!(!key.is_empty());

    // reload into new instance
    let mut store2 = QuantizedContinuum::with_name(vn);
