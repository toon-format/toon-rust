...
Adjacency Modes:
- **positive** (default): Only ⟨α, β⟩ = +1 establishes an edge (degree = 56 per root). This avoids double-counting antipodal neighbors and is the default in implementation.
- **absolute**: |⟨α, β⟩| = 1 establishes an edge (counts ±1; degree = 112 per root). Use this mode when antipodal neighbors must be kept separate.
